...
import type { Social } from '@/lib/ui-config/types'
...
export function SocialLinks({
  ...
  if (!social?.enabled || !social.links?.length) return null

  const links = social.links.filter((l) => l.url?.trim())
  ...
  return (
    <div className={cn('flex flex-wrap items-center gap-2', className)}>
      {links.map((link) => {
        const Icon = ICONS[link.platform] ?? Share2
        return (
          <a
